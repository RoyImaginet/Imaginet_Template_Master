 ...
 */
function imaginet_enable_svg_uploads( $mimes ) {
...
add_filter( 'upload_mimes', 'imaginet_enable_svg_uploads' );

/**
 * Force the WordPress Media Library to render SVG thumbnails in the dashboard
 */
function imaginet_fix_svg_media_library_previews( $response, $attachment, $meta ) {
    if ( $response['type'] === 'image' && $response['subtype'] === 'svg+xml' && method_exists( $attachment, 'guid' ) ) {
        $response['sizes'] = [
            'full' => [
                'url' => $attachment->guid,
                'width' => $response['width'] ?? 150, // Fallback width
                'height' => $response['height'] ?? 150, // Fallback height
...
add_filter( 'wp_prepare_attachment_for_js', 'imaginet_fix_svg_media_library_previews', 10, 3 );
...
//Change the Login Screen Logo Graphic
function imaginet_custom_login_logo() {
...
add_action('login_head', 'imaginet_custom_login_logo');

function imaginet_login_logo_url() {
    return home_url();
}
add_filter('login_headerurl', 'imaginet_login_logo_url');

function imaginet_login_logo_url_title() {
    return get_bloginfo('name');
}
add_filter('login_headertext', 'imaginet_login_logo_url_title');
